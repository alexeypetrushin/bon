import { assert, test, sort_by, round, fill, filter_map, find_index } from './base';
// median --------------------------------------------------------------------------------
export function median(values, is_sorted = false) {
...
// quantile ------------------------------------------------------------------------------
export function quantile(values, q, is_sorted = false) {
...
export function map_with_rank(list, order_by, map) {
    ...
    for (let i = 0; i < sorted.length; i++) {
        ...
        if (i > 0 && current.order_by != sorted[i - 1].order_by)
            rank++;
        sorted_with_rank.push(Object.assign(Object.assign({}, current), { rank }));
    }
...
test(() => {
    assert.equal(map_with_rank([4, 2, 3, 4, 5, 7, 5], (v) => v, (v, r) => [v, r]), [[4, 3], [2, 1], [3, 2], [4, 3], [5, 4], [7, 5], [5, 4]]);
});
function linear_regression_wrong(x_y_r) {
    const xyr = x_y_r.map(([x, y, r]) => [x, y, r === undefined ? 1 : r]);
    let i, x, y, r, sumx = 0, sumy = 0, sumx2 = 0, sumy2 = 0, sumxy = 0, sumr = 0, a, b;
...
export function differentiate(sparce_values) {
    const diffs = fill(sparce_values.length, undefined);
...
export function integrate(diffs, base = 1) {
    assert(diffs[0] === undefined, `first element of diff serie should always be undefined`);
    const values = fill(diffs.length, undefined);
